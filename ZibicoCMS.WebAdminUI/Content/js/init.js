...
$(function () {
    ...
    //});
    // Clear event
    //$('.image-preview-clear').click(function (e) {
    //    $(this).parent().parent().find('.image-preview').attr("data-content", "").popover('hide');
    //    $(this).parent().parent().find(".image-preview-filename").val("");
    //    $(this).hide();
    //    $(this).parent().find(".image-preview-input input:file").val("");
    //    $(this).parent().find(".image-preview-input-title").text("Seçin");
    //});
    // Create the preview image
    $(".image-preview-input input:file").change(function (data) {
        var $btn = $(this);
        var img = $('<img/>', {
            id: 'dynamic',
            width: 250,
        ...
        //}
        reader.onload = function (e) {
            $btn.parent().find(".image-preview-input-title").text("Değiştir");
            $btn.parent().parent().find(".image-preview-clear").show();
            $btn.parent().parent().parent().find(".image-preview-filename").val(file.name);
            img.attr('src', e.target.result);
            //$btn.parent().parent().parent().parent().find(".image-preview").attr("data-content", $(img)[0].outerHTML);
        }
